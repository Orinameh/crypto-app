...
const CryptoDetail = ({ route, navigation }) => {
  ...
  function renderDots() {
    const dotPosotion = Animated.divide(scrollX, SIZES.width);
      ...
      >
        ...
        >
          {numberOfCharts.map((item, index) => {
            const opacity = dotPosotion.interpolate({
              inputRange: [index - 1, index, index + 1],
              outputRange: [0.3, 1, 0.3],
              extrapolate: "clamp",
            });

            const dotSize = dotPosotion.interpolate({
              inputRange: [index - 1, index, index + 1],
              outputRange: [SIZES.base * 0.8, 10, SIZES.base * 0.8],
              extrapolate: "clamp",
            });

            const dotColor = dotPosotion.interpolate({
              inputRange: [index - 1, index, index + 1],
